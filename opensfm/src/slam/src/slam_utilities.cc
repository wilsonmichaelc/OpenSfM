...
Eigen::Matrix3d
SlamUtilities::to_skew_symmetric_mat(const Eigen::Vector3d &vec)
{
  Eigen::Matrix3d skew;
  skew << 0, -vec(2), vec(1),
      vec(2), 0, -vec(0),
...
Eigen::Matrix3d
SlamUtilities::create_E_21(const Eigen::Matrix3d &rot_1w, const Eigen::Vector3d &trans_1w,
...
SlamUtilities::ComputeMinMaxDepthInShot(const map::Shot& shot)
{
    ...
    const Eigen::Vector3d last_row = R_cw.row(2);
...
void SlamUtilities::Triangulate(const TrackId track, const TracksManager& tracks_manager, map::Map& map, const double reproj_threshold, const double min_reproj_angle)
{
  ...
  for (const auto& shot_obs : tracks_manager.GetTrackObservations(track))
  {
    ...
    if (shot != nullptr) 
    {
      Eigen::Vector3d bearing = shot->shot_camera_.camera_model_.PixelBearing(shot_obs.second.point);
