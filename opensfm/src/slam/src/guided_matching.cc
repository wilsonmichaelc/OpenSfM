...
GuidedMatcher::GuidedMatcher(const GridParameters& grid_params):
//, const BrownPerspectiveCamera& camera, SlamReconstruction* map_db):
...
void 
GuidedMatcher::DistributeUndistKeyptsToGrid(const std::vector<cv::KeyPoint>& undist_keypts, CellIndices& keypt_indices_in_cells) const
{
...
GuidedMatcher::match_frame_and_landmarks(cslam::Frame& frm, std::vector<cslam::Landmark*>& local_landmarks, const float margin)
{
    ...
    std::cout << "match_frame_and_landmarks: " << frm.im_name << "scale: " << frm.scale_factors_.size() << " local_lm: " << local_landmarks.size() << std::endl;
    ...
    for (auto local_lm : local_landmarks) {
        ...
        if (best_hamm_dist <= HAMMING_DIST_THR_HIGH) {
            ...
            std::cout << "Adding lm " << local_lm->lm_id_ << " to " << best_idx << std::endl;
            // 対応情報を追加
...
template size_t GuidedMatcher::replace_duplication(KeyFrame*, const std::set<Landmark*, LandmarkCompare>&, const float) const;
*/
};
