...
def match_unwrap_args(args):
    ...
    for im2 in candidates:
        ...
        initial_im1_im2, im1_im2 = match(im1, im2, camera1, camera2, ctx.data)
    ...
    all_im1_matches = {} if ctx.overwrite or not ctx.data.matches_exists(im1) else ctx.data.load_matches(im1)
...
def match(im1, im2, camera1, camera2, data):
    ...
    if matcher_type == 'WORDS':
        ...
        matches = csfm.match_using_cascade_hashing(f1, f2, data.config['lowes_ratio'])
