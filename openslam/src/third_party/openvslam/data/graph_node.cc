...
graph_node::graph_node(cslam::KeyFrame* keyfrm, const bool spanning_parent_is_not_set)
    : owner_keyfrm_(keyfrm), spanning_parent_is_not_set_(spanning_parent_is_not_set) {}

void graph_node::add_connection(cslam::KeyFrame* keyfrm, const unsigned int weight) {
...
void graph_node::erase_connection(cslam::KeyFrame* keyfrm) {
    bool need_update = false;
    {
        // std::lock_guard<std::mutex> lock(mtx_);
        if (connected_keyfrms_and_weights_.count(keyfrm)) {
...
void graph_node::update_connections() {
    ...
    const auto& landmarks = owner_keyfrm_->landmarks_;

    std::map<cslam::KeyFrame*, unsigned int> keyfrm_weights;
    ...
    cslam::KeyFrame* nearest_covisibility = nullptr;

    // vector for sorting
    std::vector<std::pair<unsigned int, cslam::KeyFrame*>> weight_covisibility_pairs;
...

    {
        ...
        if (spanning_parent_is_not_set_ && owner_keyfrm_->kf_id_ != 0) {
...
void graph_node::update_covisibility_orders() {
    ...
    std::vector<std::pair<unsigned int, cslam::KeyFrame*>> weight_keyfrm_pairs;
...
std::set<cslam::KeyFrame*> graph_node::get_connected_keyframes() const {
    // std::lock_guard<std::mutex> lock(mtx_);
    std::set<cslam::KeyFrame*> keyfrms;
...
std::vector<cslam::KeyFrame*> graph_node::get_covisibilities() const {
    // std::lock_guard<std::mutex> lock(mtx_);
    return ordered_covisibilities_;
}

std::vector<cslam::KeyFrame*> graph_node::get_top_n_covisibilities(const unsigned int num_covisibilities) const {
    // std::lock_guard<std::mutex> lock(mtx_);
    if (ordered_covisibilities_.size() < num_covisibilities) {
        return ordered_covisibilities_;
    }
    else {
        return std::vector<cslam::KeyFrame*>(ordered_covisibilities_.begin(), ordered_covisibilities_.begin() + num_covisibilities);
    }
}

std::vector<cslam::KeyFrame*> graph_node::get_covisibilities_over_weight(const unsigned int weight) const {
    // std::lock_guard<std::mutex> lock(mtx_);

    if (ordered_covisibilities_.empty()) {
        return std::vector<cslam::KeyFrame*>();
    }
    ...
    if (itr == ordered_weights_.end()) {
        ...
        return std::vector<cslam::KeyFrame*>(ordered_covisibilities_.begin(), ordered_covisibilities_.begin() + num);
    }
}

unsigned int graph_node::get_weight(cslam::KeyFrame* keyfrm) const {
    // std::lock_guard<std::mutex> lock(mtx_);
    if (connected_keyfrms_and_weights_.count(keyfrm)) {
...
void graph_node::set_spanning_parent(cslam::KeyFrame* keyfrm) {
    // std::lock_guard<std::mutex> lock(mtx_);
    assert(!spanning_parent_);
    spanning_parent_ = keyfrm;
}

cslam::KeyFrame* graph_node::get_spanning_parent() const {
    // std::lock_guard<std::mutex> lock(mtx_);
    return spanning_parent_;
}

void graph_node::change_spanning_parent(cslam::KeyFrame* keyfrm) {
    // std::lock_guard<std::mutex> lock(mtx_);
    spanning_parent_ = keyfrm;
    keyfrm->graph_node_->add_spanning_child(owner_keyfrm_);
}

void graph_node::add_spanning_child(cslam::KeyFrame* keyfrm) {
    // std::lock_guard<std::mutex> lock(mtx_);
    spanning_children_.insert(keyfrm);
}

void graph_node::erase_spanning_child(cslam::KeyFrame* keyfrm) {
    // std::lock_guard<std::mutex> lock(mtx_);
...
void graph_node::recover_spanning_connections() {
    ...
    std::set<cslam::KeyFrame*> new_parent_candidates;
    ...
    while (!spanning_children_.empty()) {
        ...
        unsigned int max_weight = 0;
        cslam::KeyFrame* max_weight_parent = nullptr;
        cslam::KeyFrame* max_weight_child = nullptr;
...
std::set<cslam::KeyFrame*> graph_node::get_spanning_children() const {
    // std::lock_guard<std::mutex> lock(mtx_);
    return spanning_children_;
}

bool graph_node::has_spanning_child(cslam::KeyFrame* keyfrm) const {
    // std::lock_guard<std::mutex> lock(mtx_);
    return static_cast<bool>(spanning_children_.count(keyfrm));
}

// void graph_node::add_loop_edge(cslam::KeyFrame* keyfrm) {
//     // std::lock_guard<std::mutex> lock(mtx_);
...
// std::set<cslam::KeyFrame*> graph_node::get_loop_edges() const {
//     // std::lock_guard<std::mutex> lock(mtx_);
//     return loop_edges_;
// }
...
std::vector<cslam::KeyFrame*> graph_node::extract_intersection(const T& keyfrms_1, const U& keyfrms_2) {
    std::vector<cslam::KeyFrame*> intersection;
