...
class SlamInitializer(object):
    ...
    def initialize_opensfm(self, data, frame):
        ...
        p1, f1, c1 = feature_loader.instance.\
            load_points_features_colors(data, im1, masked=True)  # data.load_features(im1)
        ...
        # success, matches1 = self.slam_matcher.match(data, im1, im2, camera)
        # print("len(matches1): ", len(matches1))
        # success, matches2 = self.slam_matcher.match(data, im2, im1, camera)
        # print("len(matches2): ", len(matches2))
        # print("matches1", matches1)
        # matches1 = np.asarray(matches1[im2])
        # matches2 = np.asarray(matches2[im1])
        # m1, m2 = matches1[0, :]
        # idx = np.argwhere(matches2[:,0] == m2)
        # print("m2: ", matches2[idx,:], m1, m2)
        ...
        # slam_debug.visualize_matches(matches, im1, im2, data)
        # exit()
        ...
        print("Created init rec with {} points from {} matches"
              .format(len(reconstruction_init.points), len(matches)))
